...
class User
{
  ...
  public function login($username, $password)
  {
    ...
    if ($result) {
      if (password_verify($password, $result['password'])) {
        ['name' => $name, 'username' => $username, 'email' => $email, 'password' => $password, 'role' => $role] = $result;
        $this->setAttributes($username, $name, $email, $password, $role);
        return true;
      }
  ...
  public function change_email($new_email)
  {
    ...
    $stmt = $this->conn->prepare($sql);
    $new_mail = htmlspecialchars(strip_tags($new_email));
    try {
